...
func (s *Service) IdentifyVoiceprint(ctx context.Context, req *IdentifyRequest) (*IdentifyResult, error) {
	...
	// 构建扩展结果
	speakerID := strings.TrimSpace(resp.FeatureID)
	if speakerID == "" {
		speakerID = strings.TrimSpace(resp.SpeakerID)
	}
...
func (s *Service) DeleteVoiceprint(ctx context.Context, speakerID string, scope ...string) (*DeleteResponse, error) {
	if !s.IsEnabled() {
		return nil, ErrServiceDisabled
	}
	...
	// 调用API删除
	resp, err := s.client.DeleteVoiceprint(ctx, speakerID, scope...)
	if err != nil {
		return nil, err
	}
...
func (s *Service) BindVoiceprintAssistant(ctx context.Context, tenantID, speakerID, assistantID string) (*DeleteResponse, error) {
	if !s.IsEnabled() {
		return nil, ErrServiceDisabled
	}
	return s.client.BindVoiceprintAssistant(ctx, tenantID, speakerID, assistantID)
}
