...
func TestNeteaseProvider_Kind(t *testing.T) {
	p, err := NewNeteaseProvider(ProviderConfig{
		"app_key": "test-key", "app_secret": "test-secret",
	})
	require.NoError(t, err)
	assert.Equal(t, ProviderNeteaseYunx, p.Kind())
}

func TestNeteaseProvider_MissingConfig(t *testing.T) {
	_, err := NewNeteaseProvider(ProviderConfig{"app_key": "test-key"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "app_key and app_secret")
}

func TestNeteaseProvider_Send_Success(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "/sms/sendtemplate.action", r.URL.Path)
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprint(w, `{"code":200,"msg":"ok","obj":"obj-123"}`)
	}))
	defer srv.Close()
	...
	assert.Equal(t, "obj-123", res.MessageID)
}

func TestNeteaseProvider_Send_Failure(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprint(w, `{"code":413,"msg":"bad template","obj":""}`)
	}))
	defer srv.Close()
...
func TestNeteaseProvider_RequiresTemplate(t *testing.T) {
	p, err := NewNeteaseProvider(ProviderConfig{
		"app_key": "test-key", "app_secret": "test-secret",
	})
	require.NoError(t, err)
...
func TestNeteaseProvider_Send_TransportError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}))
	srv.Close()
