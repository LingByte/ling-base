...
func PostForm(ctx context.Context, endpoint string, form url.Values, headers map[string]string, basicUser, basicPass string) ([]byte, error) {
	if endpoint == "" {
		return nil, fmt.Errorf("push: empty endpoint")
	}
	var body io.Reader
	if form != nil {
...
func doRequest(req *http.Request) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("push: read response: %w", err)
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return data, fmt.Errorf("push: unexpected status %d: %s", resp.StatusCode, truncate(string(data), 200))
	}
	return data, nil
}
...
func PostFormRaw(ctx context.Context, endpoint string, form url.Values, headers map[string]string, basicUser, basicPass string) (int, []byte, error) {
	if endpoint == "" {
		return 0, nil, fmt.Errorf("push: empty endpoint")
	}
	var body io.Reader
	if form != nil {
		body = strings.NewReader(form.Encode())
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, endpoint, body)
	if err != nil {
		return 0, nil, fmt.Errorf("push: build request: %w", err)
	}
	if form != nil {
		req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	}
...
func doRequestRaw(req *http.Request) (int, []byte, error) {
	...
	resp, err := defaultHTTPClient.Do(req)
	if err != nil {
		return 0, nil, fmt.Errorf("push: http request: %w", err)
	}
	defer resp.Body.Close()
	data, err := io.ReadAll(resp.Body)
	if err != nil {
		return resp.StatusCode, nil, fmt.Errorf("push: read response: %w", err)
	}
