...
// Tool is an OpenAI-style function tool definition for session.update.
type Tool struct {
	Name        string
	Description string
...
// should be delegated to another goroutine by the caller.
type ToolHandler func(name string, args map[string]any) string

// ToolsForSession builds the `tools` array for session.update.
