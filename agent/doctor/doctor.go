// Package doctor produces a diagnostic report of LingAgent's environment: which
...
type Input struct {
	...
	ConfigFound bool        // a .ling-agent/config.toml was loaded
...
func Run(in Input) []Check {
	...
	// Context window — show the limit LingAgent will request against, plus where
	...
	// Config presence.
	if in.ConfigFound {
		add("config", StatusOK, ".ling-agent/config.toml loaded")
	} else {
		add("config", StatusInfo, "no .ling-agent/config.toml (using defaults)")
	}
