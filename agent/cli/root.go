...
func buildProvider(cfg config.Config) (api.Provider, string, error) {
	switch cfg.Provider {
	case config.ProviderRelay:
		if cfg.BaseURL == "" {
			return nil, "", fmt.Errorf("provider \"relay\" requires baseURL in config (e.g. https://llmapi.qiniu.io)")
		}
		key := cfg.ResolveAPIKey()
		if key == "" {
			return nil, "", fmt.Errorf("provider \"relay\" needs apiKey or apiKeyEnv in config; if using apiKeyEnv, export that variable before running")
		}
		model := cfg.Model
		if model == "" {
			model = "gpt-5.4-mini"
		...
		maxTokens := 8192
		return api.NewRelayProviderFromConfig(cfg.BaseURL, key, model, cfg.Temperature, maxTokens), model, nil
	case config.ProviderOpenAI:
		if cfg.BaseURL == "" {
			return nil, "", fmt.Errorf("provider \"openai\" requires baseURL in ~/.ling-agent/config.toml or ./.ling-agent/config.toml (try --create-config=global or --create-config=local)")
		}
		key := cfg.ResolveAPIKey()
		if key == "" {
			return nil, "", fmt.Errorf("provider \"openai\" needs apiKey or apiKeyEnv in ~/.ling-agent/config.toml or ./.ling-agent/config.toml; if using apiKeyEnv, export that variable before running")
		}
	...
	default:
		...
		return api.New(cred, os.Getenv("LING_AGENT_CUSTOM_ENDPOINT")), cfg.Model, nil
	}
}
...
func NewRootCommand() *cobra.Command {
	...
	// Match commander's `--version` output: "<version> (LingAgent)" with no prefix.
...
func run(cmd *cobra.Command, opts *options) error {
	...
	if interactive {
		...
		sess := &tui.Session{
			...
			Theme:               themeOrWarn(cfg.Theme, func(m string) { fmt.Fprintln(cmd.ErrOrStderr(), "warning:", m) }), // user default (~/.ling-agent) overlaid by project; /theme overrides per session
			PermissionMode:      string(mode),
			Memory:              memStore,
