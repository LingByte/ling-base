...
// (streamable / SSE) and exposes their tools and resources to LingAgent. It wraps
...
// (not an error). A project .ling-agent/.mcp.json overrides ./.mcp.json per server,
// so MCP servers can be locally overridden in the .ling-agent folder.
func LoadConfig(dir string) (Config, error) {
	cfg := Config{MCPServers: map[string]ServerConfig{}}
	for _, p := range []string{
		filepath.Join(dir, ".mcp.json"),             // base
		filepath.Join(dir, ".ling-agent", ".mcp.json"), // local override (wins)
	} {
...
func newClient() *mcpsdk.Client {
	return mcpsdk.NewClient(&mcpsdk.Implementation{Name: "LingAgent", Version: version.GetVersion()}, nil)
}
