...
func TestLoadConfigAgentOverride(t *testing.T) {
	dir := t.TempDir()
	...
	// .ling-agent/.mcp.json overrides "a" and adds "c".
	os.MkdirAll(filepath.Join(dir, ".ling-agent"), 0o755)
	os.WriteFile(filepath.Join(dir, ".ling-agent", ".mcp.json"),
	...
	if cfg.MCPServers["a"].Command != "a-override" {
		t.Errorf("a = %q, want a-override (.ling-agent wins)", cfg.MCPServers["a"].Command)
	}
