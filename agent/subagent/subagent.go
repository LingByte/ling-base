...
import "github.com/LingByte/ling-base/agent/tools"
	...
	return []Type{
		{
			...
			Tools: []string{"*"},
			SystemPrompt: "You are an agent for Ling. Given the user's message, use the available tools to " +
		...
		},
		{
			...
			SystemPrompt: "You are a read-only exploration agent for Ling. You may ONLY read and search; " +
		...
		},
		{
			...
			Tools: []string{"Read", "Glob", "Grep"},
			SystemPrompt: "You are a software architect agent for Ling. Explore the codebase (read-only) and " +
