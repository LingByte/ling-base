...
func DefaultEngine(parent context.Context) *Engine {
	return NewEngine(parent, Options{
		Mode:           ModeLaunch,
		Headless:       envBool("LING_AGENT_BROWSER_HEADLESS", true),
		ChromePath:     getenv("LING_AGENT_CHROME_PATH"),
		UserDataDir:    firstNonEmpty(getenv("LING_AGENT_CHROME_USER_DATA_DIR"), DefaultUserDataDir()),
		RemoteURL:      getenv("LING_AGENT_CHROME_REMOTE_URL"),
		SearchEngine:   getenv("LING_AGENT_WEB_SEARCH_ENGINE"),
		HeadedFallback: envBool("LING_AGENT_BROWSER_HEADED_FALLBACK", true),
	})
}
