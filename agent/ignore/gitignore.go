// Package ignore provides a minimal .gitignore matcher for LingAgent's
// file-walking tools (Glob, Grep, Read). It is intentionally small: enough
// to drop obvious non-source directories (build outputs, dependency and
// tool caches) from recursive walks, not a faithful git reimplementation.
package ignore
