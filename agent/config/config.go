// Package config loads ling-agent settings from .ling-agent/config.toml — a project
// .ling-agent/ (in the working directory) overlaid on the user's ~/.ling-agent/.
...
const (
	...
	ProviderRelay     = "relay"     // ling-base/relay (40+ provider adaptors)
)

// Config is the .ling-agent/config.toml schema.
type Config struct {
...
type Browser struct {
	...
	// UserDataDir stores Chrome profile state/cookies. Defaults to ~/.ling-agent/browser/chrome-profile.
...
// ProjectDir returns cwd/.ling-agent.
func ProjectDir(cwd string) string {
	return filepath.Join(cwd, ".ling-agent")
}

// ProjectPath returns cwd/.ling-agent/config.toml.
func ProjectPath(cwd string) string {
	return filepath.Join(ProjectDir(cwd), "config.toml")
}

// ProjectDirExists reports whether cwd/.ling-agent exists and is a directory.
func ProjectDirExists(cwd string) bool {
	st, err := os.Stat(ProjectDir(cwd))
	return err == nil && st.IsDir()
}

// AppendProjectPermission appends rule to cwd/.ling-agent/config.toml under
// permissions.allow or permissions.deny. It is a no-op when cwd/.ling-agent does
...
// Load reads ~/.ling-agent/config.toml then overlays ./.ling-agent/config.toml
// (project settings win). Missing files are ignored.
func Load(cwd string) Config {
	var cfg Config
	if home, err := os.UserHomeDir(); err == nil {
		merge(&cfg, read(filepath.Join(home, ".ling-agent", "config.toml")))
	}
