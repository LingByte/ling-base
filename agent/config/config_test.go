...
func writeConfig(t *testing.T, dir, body string) {
	t.Helper()
	kd := filepath.Join(dir, ".ling-agent")
...
func TestResolveAPIKey(t *testing.T) {
	...
	t.Setenv("LING_AGENT_TEST_KEY", "from-env")
	if (Config{APIKeyEnv: "LING_AGENT_TEST_KEY"}).ResolveAPIKey() != "from-env" {
...
func TestAppendProjectPermission(t *testing.T) {
	...
	if ok, err := AppendProjectPermission(cwd, "allow", "Edit"); err != nil || ok {
		t.Fatalf("AppendProjectPermission without .ling-agent = %v,%v, want false,nil", ok, err)
	}

	if err := os.MkdirAll(filepath.Join(cwd, ".ling-agent"), 0o755); err != nil {
		t.Fatal(err)
	}
	if ok, err := AppendProjectPermission(cwd, "allow", "Edit"); err != nil || !ok {
