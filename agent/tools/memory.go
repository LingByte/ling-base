...
type MemoryInput struct {
	...
	Scope       string `json:"scope,omitempty" jsonschema:"enum=session,enum=project,description=Where to write for operation=add: session (default) writes .ling-agent/MEMORY.md; project writes .ling-agent/KNOWLEDGE.md"`
...
// project-scope writes to .ling-agent/KNOWLEDGE.md.
...
func (m *Memory) Description(context.Context) (string, error) {
	return "Your persistent memory across sessions. Use it to recall prior context before " +
		...
		".ling-agent/memory/*.md detail notes; a detail hit is tagged \"file.md: …\", Read " +
...
func (m *Memory) Execute(_ context.Context, _ Context, raw json.RawMessage) ([]Result, error) {
	...
	case "add":
		if in.Scope == "project" {
			...
			return []Result{{Content: "Stored to project knowledge (.ling-agent/KNOWLEDGE.md)."}}, nil
		}
