...
func TestAugmentSystemPrependsBillingHeaderForOAuth(t *testing.T) {
	...
	// are server-logged but not validated, so we ship LingAgent branding honestly.
	c := &Client{cred: Credential{AuthToken: "fake-oauth"}}
	...
	if !strings.Contains(got[0].Text, "cc_entrypoint=ling-agent") {
		t.Errorf("billing entrypoint must declare LingAgent honestly; got %q", got[0].Text)
	}
...
func TestOAuthRequestCarriesBillingHeaderInBody(t *testing.T) {
	t.Setenv("LING_AGENT_MAX_RETRIES", "0")
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantPrefix {
				...
				if !strings.Contains(got.System[0].Text, "cc_entrypoint=ling-agent") {
					t.Errorf("billing block should declare LingAgent honestly; got %q", got.System[0].Text)
				}
			} else {
