...
func ResolveCredential() (Credential, error) {
	...
	return Credential{}, fmt.Errorf(`LingAgent needs credentials before it can start.
...
Choose one:
  ...
  2. Existing Claude Code login on macOS:
     claude
     # sign in there, then run ling-agent again

  3. OpenAI-compatible provider config:
     ling-agent --create-config=global   # writes ~/.ling-agent/config.toml
     # or: ling-agent --create-config=local  # writes ./.ling-agent/config.toml
     # edit provider/baseURL/model/apiKeyEnv, export that env var, then run ling-agent

Global config (~/.ling-agent/config.toml) is loaded automatically; local project config
(./.ling-agent/config.toml) overlays it when present.

Tip: after setup, run /doctor inside LingAgent to verify auth and environment status.`)
}
