...
//
// We deliberately use the Beta Messages surface because LingAgent relies on beta
...
// modelContextWindows is the per-model input-token limit LingAgent would actually
...
// Client is LingAgent's Anthropic API client: the SDK client plus the resolved
...
// and watching them all 200), so we put honest LingAgent metadata in there.
...
func (c *Client) augmentSystem(in []anthropic.BetaTextBlockParam) []anthropic.BetaTextBlockParam {
	...
	billing := anthropic.BetaTextBlockParam{
		Text: claudeCodeBillingPrefix + " cc_version=" + claudeCodeVersion + "; cc_entrypoint=ling-agent;",
	}
...
// maxRetries resolves the retry count, overridable via LING_AGENT_MAX_RETRIES.
func maxRetries() int {
	if v := os.Getenv("LING_AGENT_MAX_RETRIES"); v != "" {
