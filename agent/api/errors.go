...
func FriendlyError(err error) string {
	...
	if status, ok := apiStatus(err); ok {
		...
		case 429:
			...
			if oai := openAIPayload(err); oai != nil {
				...
				if oai.Message != "" {
					return "Rate limited (429): " + strings.TrimSpace(oai.Message) +
						" (Set LING_AGENT_MAX_RETRIES to retry more.)"
				}
			}
			...
			if errType, msg := anthropicPayload(err); errType != "" || msg != "" {
				...
				if detail := meaningfulMessage(msg, errType); detail != "" {
					...
					if isLongContextCredits(detail) {
						return out + ": " + detail + " (Retries won't help — this needs pay-as-you-go usage credits for the long-context tier. Add credits, or reduce context: lower `contextWindow` in .ling-agent/config.toml so autocompaction triggers earlier, and run /compact.)"
					}
					return out + ": " + detail + " (If you signed in via Claude Code OAuth this often happens when the token is in use by another session. Set LING_AGENT_MAX_RETRIES to retry more.)"
				}
				return out + ": if you signed in via Claude Code OAuth this often happens when the token is in use by another session — try closing other sessions and waiting a moment. (Set LING_AGENT_MAX_RETRIES to retry more.)"
			}
			...
			return out + "Wait a moment and try again. (Set LING_AGENT_MAX_RETRIES to retry more.)"
		case 400:
			...
			if detail := providerDetail(err); detail != "" {
				if isContextOverflow(detail) {
					return "Conversation outgrew the model's context window — the provider returned: " + detail +
						" Set `contextWindow` in .ling-agent/config.toml to match this model so autocompaction triggers earlier, and run /compact to summarise the current session."
				}
		...
		case 401, 403:
			...
			if detail := providerDetail(err); detail != "" {
				return fmt.Sprintf("Authentication failed (%d): %s. Check your API key / sign-in (or .ling-agent/config.toml for a custom provider).", status, detail)
			}
			return fmt.Sprintf("Authentication failed (%d): check your API key / sign-in "+
				"(or .ling-agent/config.toml for a custom provider).", status)
		case 529:
			if detail := providerDetail(err); detail != "" {
	...
	if errors.As(err, &dnsErr) || errors.As(err, &netErr) {
		return "Could not reach the model endpoint (network error). Check your internet " +
			"connection and the baseURL in ~/.ling-agent/config.toml or ./.ling-agent/config.toml.\n" +
			"Details: " + err.Error()
	}
	...
	if errors.Is(err, ErrStreamStalled) {
		return "The model connection stalled (no data received before the idle timeout) and was " +
			...
			"streaming connection. Send your message again; tune the window with LING_AGENT_STREAM_IDLE_TIMEOUT " +
			"(seconds, 0 disables)."
	}
