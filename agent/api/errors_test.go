...
func TestFriendlyError429AnthropicSurfacesUpstreamMessage(t *testing.T) {
	...
	for _, want := range []string{
		...
		"LING_AGENT_MAX_RETRIES", // the retry knob is still relevant
	} {
...
func TestFriendlyError429HintIsProviderSpecific(t *testing.T) {
	...
	if !strings.Contains(openAI, "LING_AGENT_MAX_RETRIES") {
...
func TestFriendlyErrorInsufficientQuotaDoesNotRecommendRetry(t *testing.T) {
	...
	// drop the misleading "retries / LING_AGENT_MAX_RETRIES" advice.
	...
	for _, banned := range []string{"LING_AGENT_MAX_RETRIES", "retries were exhausted", "Wait a moment"} {
...
func TestFriendlyError429PassesThroughOpenAIMessage(t *testing.T) {
	...
	if !strings.Contains(msg, "LING_AGENT_MAX_RETRIES") {
...
func TestFriendlyErrorPassesThroughUpstreamDetail(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			wantSubs: []string{"Authentication failed (401)", "Incorrect API key provided", "Check your API key", ".ling-agent/config.toml"},
		},
		{
...
func TestMaxRetriesEnvOverride(t *testing.T) {
	t.Setenv("LING_AGENT_MAX_RETRIES", "9")
	if maxRetries() != 9 {
		t.Errorf("maxRetries = %d, want 9", maxRetries())
	}
	t.Setenv("LING_AGENT_MAX_RETRIES", "")
...
func TestFriendlyError429SessionLimitAdvisesNotToRetry(t *testing.T) {
	...
	// and the "set LING_AGENT_MAX_RETRIES" suggestion is actively wrong since
	...
	for _, forbidden := range []string{"token is in use by another session", "LING_AGENT_MAX_RETRIES"} {
...
func TestFriendlyError429LongContextCreditsAdvisesNotToRetry(t *testing.T) {
	...
	// "set LING_AGENT_MAX_RETRIES" and OAuth-sharing hints are both wrong. Detect
