...
// TUI loop and the headless `ling-agent --loop` share them.
package goal
...
// Preference order: ./PRD.md, then ./.ling-agent/GOAL.md. When neither exists it
// returns the default create path (.ling-agent/GOAL.md) with found=false.
func SpecPath(cwd string) (path string, found bool) {
	candidates := []string{
		filepath.Join(cwd, "PRD.md"),
		filepath.Join(cwd, ".ling-agent", "GOAL.md"),
	}
