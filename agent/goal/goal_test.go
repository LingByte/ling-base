...
func TestSpecPathPrecedence(t *testing.T) {
	t.Run("none exists returns default create path", func(t *testing.T) {
		...
		if want := filepath.Join(dir, ".ling-agent", "GOAL.md"); path != want {
			t.Errorf("default path = %q, want %q", path, want)
		}
	})

	t.Run("PRD.md wins over .ling-agent/GOAL.md", func(t *testing.T) {
		dir := t.TempDir()
		os.MkdirAll(filepath.Join(dir, ".ling-agent"), 0o755)
		os.WriteFile(filepath.Join(dir, ".ling-agent", "GOAL.md"), []byte("# Goal\n"), 0o644)
	...
	t.Run("falls back to .ling-agent/GOAL.md", func(t *testing.T) {
		dir := t.TempDir()
		os.MkdirAll(filepath.Join(dir, ".ling-agent"), 0o755)
		os.WriteFile(filepath.Join(dir, ".ling-agent", "GOAL.md"), []byte("# Goal\n"), 0o644)

		path, found := SpecPath(dir)
		if !found || path != filepath.Join(dir, ".ling-agent", "GOAL.md") {
			t.Errorf("got (%q, %v), want .ling-agent/GOAL.md found", path, found)
		}
	})
}
...
func TestMergeHint(t *testing.T) {
	withBase := MergeHint("ling-agent/goal-x", "main")
	if !contains(withBase, "git diff main...ling-agent/goal-x") || !contains(withBase, "git merge ling-agent/goal-x") {
		t.Errorf("hint with base = %q", withBase)
	}
	// Unknown base (empty, or same as the branch) → generic guidance.
	for _, base := range []string{"", "ling-agent/goal-x"} {
		g := MergeHint("ling-agent/goal-x", base)
...
func TestPromptsReferenceSpecPath(t *testing.T) {
	const p = "/repo/.ling-agent/GOAL.md"
