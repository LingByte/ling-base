...
func TestSlashCommandsDuringProcessing(t *testing.T) {
	...
	if !strings.Contains(m.transcript.String(), "isn't available while LingAgent is working") {
...
func TestGoalLoopMergeHintOnStop(t *testing.T) {
	...
	m.loopSpecPath = writeSpec(t, "- [x] done\n")
	m.loopBranch, m.loopBaseBranch = "ling-agent/goal-x", "main"
	...
	if !strings.Contains(m.transcript.String(), "git merge ling-agent/goal-x") {
