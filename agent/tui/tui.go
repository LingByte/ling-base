...
func intro(model, branch, tagline string) string {
	logo := logoStyle.Render("✦ LingAgent")
...
func newPromptInput() textarea.Model {
	in := textarea.New()
	in.Placeholder = "Ask LingAgent… (Enter to send, Ctrl+J for newline, Ctrl+C to quit)"
	in.Prompt = ""
...
var commandList = []cmdInfo{
	...
	{"/mode", "[name]", "Change how LingAgent asks permission (no arg = picker)"},
	...
	{"/quit", "", "Exit LingAgent (alias /exit)"},
}
...
func (m *Model) busyGuard(what string) bool {
	if m.state == stateRunning {
		m.appendLine(toolStyle.Render("  " + what + " isn't available while LingAgent is working — press Esc to interrupt first."))
		return true
	}
	return false
}
...
// spec (PRD.md / .ling-agent/GOAL.md) or invites the user to describe the goal so
...
func (m *Model) startGoalLoop(args []string) (tea.Model, tea.Cmd) {
	...
	if strings.TrimSpace(specText) == "" {
		m.appendLine(errStyle.Render("No goal spec found. Run /goal first to create one (PRD.md or .ling-agent/GOAL.md)."))
		return m, nil
	}
...
func (m *Model) handleSlash(input string) (tea.Model, tea.Cmd) {
	...
	case "/theme":
		if len(args) == 0 {
			if m.state == stateRunning {
				m.appendLine(toolStyle.Render("  use /theme <name> while LingAgent is working (the picker needs an idle session). Names: " + themeNames()))
				break
			}
	...
	case "/mcp":
		...
		if len(servers) == 0 {
			m.appendLine(bannerStyle.Render("No MCP servers configured. Add them in .mcp.json or .ling-agent/.mcp.json."))
			break
		}
		var b strings.Builder
	...
	case "/status":
		...
		resume := ""
		if m.sess.SessionID != "" {
			resume = "\nresume: ling-agent --resume " + m.sess.SessionID
		}
	...
	case "/mode":
		...
		m.startChoice("Permission mode — choose how LingAgent asks before acting:", m.modeChoices())
		return m, nil
	case "/config":
...
func (m *Model) exportTranscript() (string, error) {
	...
	name := fmt.Sprintf("ling-agent-export-%s.md", time.Now().Format("20060102-150405"))
...
// when the project has a .ling-agent directory, persists it to .ling-agent/config.toml.
func (m *Model) rememberPermission(kind string, rule permission.Rule) {
	...
	if err != nil {
		...
		msg += "; saved to .ling-agent/config.toml"
	}
