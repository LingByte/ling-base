...
func runGoalLoop(ctx context.Context, errOut io.Writer, p loopRun) error {
	specText, specPath, _ := goal.Read(p.cwd)
	if strings.TrimSpace(specText) == "" {
		return fmt.Errorf("no goal spec found — create PRD.md or .ling-agent/GOAL.md first (run ling-agent interactively and use /goal)")
	}
