// Package session reads and writes ling-agent session transcripts:
// newline-delimited JSON (JSONL) under ~/.ling-agent/sessions/<encoded-cwd>/.
package session
...
// ConfigRoot returns ~/.ling-agent (honoring LING_AGENT_CONFIG_DIR).
func ConfigRoot() string {
	base := os.Getenv("LING_AGENT_CONFIG_DIR")
	if base == "" {
		home, _ := os.UserHomeDir()
		base = filepath.Join(home, ".ling-agent")
	}
	return base
}

// SessionsRoot returns ~/.ling-agent/sessions (honoring LING_AGENT_CONFIG_DIR).
