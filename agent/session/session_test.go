...
// TestEncodePath is kept stable so existing LingAgent transcripts remain under the
// same project directory names when moving storage roots.
func TestEncodePathMatchesJS(t *testing.T) {
	got := EncodePath("/Users/nickglynn/Projects/claude-code/ling-agent")
	want := "-Users-nickglynn-Projects-claude-code-ling-agent"
	if got != want {
...
func TestDirUsesSessionsRoot(t *testing.T) {
	root := t.TempDir()
	t.Setenv("LING_AGENT_CONFIG_DIR", root)
	cwd := "/work/proj"
...
func TestMostRecentSkipsContentlessSessions(t *testing.T) {
	...
	// LingAgent, an external tool, or an interrupted write. It must not shadow the
