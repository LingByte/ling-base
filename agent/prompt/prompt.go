// Package prompt assembles LingAgent's system prompt: base agent instructions,
// a security clause, live environment context (cwd, git, platform, date), and
// any AGENTS.md project instructions. A richer prompt makes LingAgent a
// materially better coding agent.
package prompt
...
const base = `You are Ling, an interactive CLI coding agent powered by ling-base. Use the instructions below and the tools available to you to assist the user.
...
func recalledKnowledge(cwd string) string {
	data, err := os.ReadFile(filepath.Join(cwd, ".ling-agent", "KNOWLEDGE.md"))
	if err != nil {
		return ""
	}
...
// "## Linked memory" section pointing at the .ling-agent/memory/*.md detail notes
...
func recalledMemory(cwd string) string {
	agentDir := filepath.Join(cwd, ".ling-agent")
	parts := readMarkdownFiles(filepath.Join(agentDir, "MEMORY.md"))

	// Backward compatibility for projects that still have the old session-memory
	// index under .ling-agent/memory/MEMORY.md.
	parts = append(parts, readMarkdownFiles(filepath.Join(agentDir, "memory", "MEMORY.md"))...)
...
// loadProjectInstructions concatenates AGENTS.md from the user global config,
...
func loadProjectInstructions(cwd string) string {
	...
	if home, err := os.UserHomeDir(); err == nil {
		paths = append(paths, filepath.Join(home, ".ling-agent", "AGENTS.md"))
	}
	if root, err := runGit(cwd, "rev-parse", "--show-toplevel"); err == nil {
		paths = append(paths, filepath.Join(strings.TrimSpace(root), "AGENTS.md"))
	}
	paths = append(paths, filepath.Join(cwd, "AGENTS.md"))
