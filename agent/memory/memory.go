...
// in .ling-agent/MEMORY.md; detailed memory files live in .ling-agent/memory/*.md;
// project knowledge lives in .ling-agent/KNOWLEDGE.md.
package memory
...
// ".ling-agent")) and call its Path() method. Kept as a thin shim so the
// LLM-facing tool surface in internal/tools/memory.go doesn't churn.
func KnowledgePath(cwd string) string {
	return NewKnowledge(filepath.Join(cwd, ".ling-agent")).Path()
}

// AddKnowledge appends a timestamped bullet to .ling-agent/KNOWLEDGE.md, creating
// .ling-agent and a header on first write. Empty text returns ErrEmpty.
//
// Deprecated: construct a Knowledge via NewKnowledge(filepath.Join(cwd,
// ".ling-agent")) and call its Add() method. Kept as a thin shim so the
// LLM-facing tool surface in internal/tools/memory.go doesn't churn.
func AddKnowledge(cwd, text string) error {
	return NewKnowledge(filepath.Join(cwd, ".ling-agent")).Add(text)
}
