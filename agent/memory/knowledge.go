...
// .ling-agent/KNOWLEDGE.md. Separate from Store because its lifecycle is
...
// the same dir as fsStore (typically ".ling-agent"), it reads and appends to
...
// from the same .ling-agent directory.
func NewKnowledge(dir string) Knowledge {
	return &FSKnowledge{dir: dir}
}

// Path returns the .ling-agent/KNOWLEDGE.md path.
