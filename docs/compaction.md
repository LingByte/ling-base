...
How LingAgent keeps a conversation within the model's context window.
...
When the estimated token count exceeds the compaction threshold, LingAgent asks
...
`~/.ling-agent/sessions/<encoded-cwd>/<id>.summary.md`. On resume, LingAgent seeds the
