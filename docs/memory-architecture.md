...
How ling-agent's auto-memory works today, and exactly what's needed to swap the
...
The shape ling-agent uses — a short always-loaded index + detail notes opened
...
ling-agent can swap backends at config time without touching any agent
code.
...
they encode "this is what `.ling-agent/MEMORY.md` looks like on disk",
...
Postgres instance, ling-agent can adopt it as a memory backend.
...
name    = "ling-agent/my-project"     # tenant label, default = cwd
```
...
- Add a `ling-agent memory migrate --to postgres` subcommand that walks
  `.ling-agent/memory/*.md` and calls pgmarkdown's `pgmd.ingest_dir`
  (PRD Phase 7).
...
Once the PG backend ships, multiple ling-agent instances (and any other
