...
| 🔀 divergent | Intentionally differs from JS (a deliberate LingAgent design choice). |
...
| Feature | JS ref | LingAgent pkg | Status | Notes |
| --- | --- | --- | --- | --- |
...
| Memory | 07-app-features | `tools/memory.go` + `memory/` | 🔀 divergent | Auto-memory + recall via `.ling-agent/memory/`. |
...
| Feature | JS ref | LingAgent pkg | Status | Notes |
| --- | --- | --- | --- | --- |
| JSONL transcripts | 07-app-features | `session/` | ✅ done | `~/.ling-agent/sessions/<encoded-cwd>/` (honors `KLAUDIA_CONFIG_DIR`); reads legacy `~/.ling-agent/projects/<encoded-cwd>/`; EncodePath matches JS K0A. |
...
| Persisted summaries + resume seeding | — | `session/summary.go` + `agent.OnSummary` | 🔀 divergent | Compaction summaries persisted to `.ling-agent/sessions/<id>.summary.md`; `--resume` seeds from them (token-saving), `--full` replays the transcript. |
| Project knowledge recall (KNOWLEDGE.md) | — | `prompt.recalledKnowledge` | 🔀 divergent | `.ling-agent/KNOWLEDGE.md` injected into the system prompt; curated, distinct from free-form memory. |
...
| Feature | JS ref | LingAgent pkg | Status | Notes |
| --- | --- | --- | --- | --- |
| MCP stdio client | 07-app-features | `mcp/` | ✅ done | `.mcp.json` + `.ling-agent/.mcp.json` override. |
...
| Command | LingAgent | Status | Notes |
| --- | --- | --- | --- |
...
| Feature | JS ref | LingAgent pkg | Status | Notes |
| --- | --- | --- | --- | --- |
| Skill system (`.ling-agent/skills/*.md`) | 07-app-features | `skill/` + `tools/skill.go` | ✅ done | Frontmatter skills as a Skill tool + TUI /<skill>. |
