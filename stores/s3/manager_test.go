...
// TestStore_ImplementsInterfaces verifies that the S3 Store implements
// both ObjectStorageManager and MultipartUploader. These are compile-time
// checks — no live S3 connection is needed.
func TestStore_ImplementsInterfaces(t *testing.T) {
	...
	if !stores.SupportsManagement(s) {
		t.Error("SupportsManagement should return true for S3 Store")
	}
	if stores.AsManager(s) == nil {
		t.Error("AsManager should return non-nil for S3 Store")
	}
	if !stores.SupportsMultipart(s) {
		t.Error("SupportsMultipart should return true for S3 Store")
	}
	if stores.AsMultipartUploader(s) == nil {
		t.Error("AsMultipartUploader should return non-nil for S3 Store")
	}
}
