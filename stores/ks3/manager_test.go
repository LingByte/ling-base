...
// TestStore_ImplementsInterfaces verifies that the KS3 Store implements
// both ObjectStorageManager and MultipartUploader. These are compile-time
// checks — no live KS3 connection is needed.
func TestStore_ImplementsInterfaces(t *testing.T) {
	...
	if !stores.SupportsManagement(s) {
		t.Error("SupportsManagement should return true for KS3 Store")
	}
	if stores.AsManager(s) == nil {
		t.Error("AsManager should return non-nil for KS3 Store")
	}
	if !stores.SupportsMultipart(s) {
		t.Error("SupportsMultipart should return true for KS3 Store")
	}
	if stores.AsMultipartUploader(s) == nil {
		t.Error("AsMultipartUploader should return non-nil for KS3 Store")
	}
}
...
func TestCreateBucket_InvalidRequest(t *testing.T) {
	s := New(Config{Region: "cn-beijing-1", AccessKeyID: "ak", AccessKeySecret: "sk"})
