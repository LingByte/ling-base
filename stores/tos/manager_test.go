...
// TestStore_ImplementsInterfaces verifies that the TOS Store implements
// both ObjectStorageManager and MultipartUploader. These are compile-time
// checks — no live TOS connection is needed.
func TestStore_ImplementsInterfaces(t *testing.T) {
	...
	if !stores.SupportsManagement(s) {
		t.Error("SupportsManagement should return true for TOS Store")
	}
	if stores.AsManager(s) == nil {
		t.Error("AsManager should return non-nil for TOS Store")
	}
	if !stores.SupportsMultipart(s) {
		t.Error("SupportsMultipart should return true for TOS Store")
	}
	if stores.AsMultipartUploader(s) == nil {
		t.Error("AsMultipartUploader should return non-nil for TOS Store")
	}
}
...
func TestCreateBucket_InvalidRequest(t *testing.T) {
	s := New(Config{Region: "cn-beijing", AccessKeyID: "ak", AccessKeySecret: "sk"})
