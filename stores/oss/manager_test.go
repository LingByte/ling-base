...
// TestStore_ImplementsInterfaces verifies that the OSS Store implements
// both ObjectStorageManager and MultipartUploader. These are compile-time
// checks — no live OSS connection is needed.
func TestStore_ImplementsInterfaces(t *testing.T) {
	...
	if !stores.SupportsManagement(s) {
		t.Error("SupportsManagement should return true for OSS Store")
	}
	if stores.AsManager(s) == nil {
		t.Error("AsManager should return non-nil for OSS Store")
	}
	if !stores.SupportsMultipart(s) {
		t.Error("SupportsMultipart should return true for OSS Store")
	}
	if stores.AsMultipartUploader(s) == nil {
		t.Error("AsMultipartUploader should return non-nil for OSS Store")
	}
}
...
func TestCreateBucket_InvalidRequest(t *testing.T) {
	s := New(Config{AccessKeyID: "ak", AccessKeySecret: "sk", Endpoint: "oss-cn-hangzhou.aliyuncs.com"})
