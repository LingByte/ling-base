...
func TestClient_Chat(t *testing.T) {
	...
	m := meter.NewMemoryMeter(nil)
	client := llm.New(
		llm.WithProvider(openai.NewProvider("test-key", openai.WithBaseURL(server.URL))),
		llm.WithMeter(m),
	)
...
func TestClient_Chat_Error(t *testing.T) {
	...
	client := llm.New(
		llm.WithProvider(openai.NewProvider("bad-key", openai.WithBaseURL(server.URL))),
	)
...
func TestClient_Embed(t *testing.T) {
	...
	resp, err := client.Embed(ctx, &llm.EmbedRequest{
		Model: "text-embedding-3-small",
		Input: input,
	})
...
func TestClient_Image(t *testing.T) {
	...
	ctx := context.Background()
	n := 2
	resp, err := client.Image(ctx, &llm.ImageRequest{
		Model:  "dall-e-3",
	...
	// Verify cost was calculated (dall-e-3 has fixed price $0.04/call).
	stats, err := m.Query(ctx, nil)
	require.NoError(t, err)
	assert.InDelta(t, 0.08, stats.TotalUsage.Cost, 0.001) // 2 images × $0.04
}
...
func TestOpenAIAdaptor_GetRequestURL(t *testing.T) {
	...
	info := &llm.ChatRequest{Model: "gpt-4o"}
	_ = info
