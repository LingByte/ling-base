...
if (state.waiting_for === "day_speech") {
  addPublicSpeech(state, state.player_seat, latestUserText());
  state.speech_index = Number(state.speech_index || 0) + 1;
  state.waiting_for = "";
}
const order = Array.isArray(state.speaker_order) && state.speaker_order.length ? state.speaker_order : aliveSeats(state);
if (Number(state.speech_index || 0) >= order.length) {
  state.phase = "vote";
  state.vote_index = 0;
  state.vote_pending = aliveSeats(state);
  state.current_votes = [];
  state.waiting_for = "";
  ...
  if (seat === Number(state.player_seat)) {
    state.waiting_for = "day_speech";
    host.emit("token", { content: "轮到" + seat + "号你发言。请以你的身份视角说明你信谁、怀疑谁，以及理由。" });
    syncVars(state);
  } else {
