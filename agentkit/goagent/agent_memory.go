...
// It restores the system prompt and short-term gomodel.
...
func (a *Agent) retrieveContext(ctx context.Context, sessionID, query string, limit int) ([]memmodel.MemoryRecord, error) {
	if a.Shared != nil {
...
func (a *Agent) SetSharedSpaces(shared *gomemory.SharedSession) {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.Shared = shared
}
...
func (a *Agent) EnsureSpaceGrants(sessionID string, spaces []string) {
	...
	for _, s := range spaces {
		...
		a.memory.Spaces.Grant(s, sessionID, gosession.SpaceRoleWriter, 0)
	}
}

// SessionMemory exposes the underlying session memory (useful for advanced setup/tests).
func (a *Agent) SessionMemory() *gomemory.SessionMemory {
	return a.memory
}
