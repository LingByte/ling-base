...
import (
	...
	"github.com/LingByte/ling-base/agentkit/memory/goengine"
...
var (
	newMemoryBankWithStore = gomemory.NewMemoryBankWithStore
	newSessionMemory       = gomemory.NewSessionMemory
	newEngine              = goengine.NewEngine
	newSharedSession       = gomemory.NewSharedSession
	newPostgresStore       = memorystore.NewPostgresStore
...
func memoryOptions(opts *gomemory.Options) gomemory.Options {
	if opts == nil {
		return gomemory.DefaultOptions()
	}
	return *opts
}

func memoryBundle(store memorystore.VectorStore, window int, embedder goembed.Embedder, opts *gomemory.Options) kit.MemoryBundle {
	if window <= 0 {
	...
	shared := func(local string, spaces ...string) *gomemory.SharedSession {
		return newSharedSession(mem, local, spaces...)
	}
	return kit.MemoryBundle{Session: mem, Shared: shared}
}

func eagerMemoryModule(name string, store memorystore.VectorStore, window int, embedder goembed.Embedder, opts *gomemory.Options) *MemoryModule {
...
func lazyMemoryModule(name string, factory memoryStoreFactory, window int, embedder goembed.Embedder, opts *gomemory.Options) *MemoryModule {
	var (
		once    sync.Once
...
func StaticMemoryProvider(mem *gomemory.SessionMemory) kit.MemoryProvider {
	shared := func(local string, spaces ...string) *gomemory.SharedSession {
		if mem == nil {
			return nil
		}
		return gomemory.NewSharedSession(mem, local, spaces...)
	}
...
func InMemoryMemoryModule(window int, embedder goembed.Embedder, opts *gomemory.Options) *MemoryModule {
	return eagerMemoryModule("memory", memorystore.NewInMemoryStore(), window, embedder, opts)
}

func InQdrantMemory(window int, baseURL string, collection string, embedder goembed.Embedder, opts *gomemory.Options) *MemoryModule {
	return eagerMemoryModule("qdrant", memorystore.NewQdrantStore(baseURL, collection, ""), window, embedder, opts)
}

func InPostgresMemory(ctx context.Context, window int, connStr string, embedder goembed.Embedder, opts *gomemory.Options) *MemoryModule {
...
func InMongoMemory(ctx context.Context, window int, uri, database, collection string, embedder goembed.Embedder, opts *gomemory.Options) *MemoryModule {
	factory := func() (memorystore.VectorStore, error) {
...
func InNeo4jMemory(ctx context.Context, window int, factory func(context.Context) (*neo4jstore.Neo4jStore, error), embedder goembed.Embedder, opts *gomemory.Options) *MemoryModule {
