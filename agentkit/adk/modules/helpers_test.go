...
import (
	...
	kit "github.com/LingByte/ling-base/agentkit/adk"
...
func (stubVectorStore) SearchMemory(context.Context, string, []float32, int) ([]memmodel.MemoryRecord, error) {
	return nil, nil
}
...
func (stubVectorStore) Iterate(context.Context, func(memmodel.MemoryRecord) bool) error {
	return nil
}

func (stubVectorStore) Count(context.Context) (int, error) { return 0, nil }

var _ storepkg.VectorStore = (*stubVectorStore)(nil)
...
var _ storepkg.SchemaInitializer = (*stubSchemaStore)(nil)
...
func TestInMemoryMemoryModule(t *testing.T) {
	opts := gomemory.DefaultOptions()
	module := InMemoryMemoryModule(0, goembed.DummyEmbedder{}, &opts)
	...
	module = InMemoryMemoryModule(3, goembed.DummyEmbedder{}, &opts)
	bundle, err = module.provider(context.Background())
	if err != nil {
		t.Fatalf("provider returned error: %v", err)
	}
...
func TestInQdrantMemory(t *testing.T) {
	opts := gomemory.DefaultOptions()
	module := InQdrantMemory(0, "http://localhost:6333", "collection", goembed.DummyEmbedder{}, &opts)
	bundle, err := module.provider(context.Background())
	if err != nil {
		t.Fatalf("provider returned error: %v", err)
	}
	...
	module = InQdrantMemory(4, "http://localhost:6333", "collection", goembed.DummyEmbedder{}, &opts)
	bundle, err = module.provider(context.Background())
	if err != nil {
...
func TestMemoryModulesAcceptNilOptions(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "in-memory",
			module: func() *MemoryModule {
				return InMemoryMemoryModule(4, goembed.DummyEmbedder{}, nil)
			},
		},
		{
			name: "qdrant",
			module: func() *MemoryModule {
				return InQdrantMemory(4, "http://localhost:6333", "collection", goembed.DummyEmbedder{}, nil)
			},
		},
		{
			name: "neo4j",
			module: func() *MemoryModule {
				return InNeo4jMemory(context.Background(), 4, func(context.Context) (*storepkg.Neo4jStore, error) {
					return &storepkg.Neo4jStore{}, nil
				}, goembed.DummyEmbedder{}, nil)
			},
		},
	}
...
func TestInPostgresMemoryCachesError(t *testing.T) {
	...
	var calls int32
	newPostgresStore = func(context.Context, string) (*storepkg.PostgresStore, error) {
		atomic.AddInt32(&calls, 1)
		return nil, errors.New("connect failed")
	}

	opts := gomemory.DefaultOptions()
	module := InPostgresMemory(context.Background(), 4, "postgres://invalid", goembed.DummyEmbedder{}, &opts)
...
func TestInPostgresMemoryInitializesOnceConcurrently(t *testing.T) {
	...
	newPostgresStore = func(context.Context, string) (*storepkg.PostgresStore, error) {
		...
		return &storepkg.PostgresStore{}, nil
	}

	module := InPostgresMemory(context.Background(), 4, "postgres://test", goembed.DummyEmbedder{}, nil)
	const workers = 32
	results := make([]*gomemory.SessionMemory, workers)
...
func TestInPostgresMemorySuccess(t *testing.T) {
	...
	newPostgresStore = func(context.Context, string) (*storepkg.PostgresStore, error) {
		return (*storepkg.PostgresStore)(nil), nil
	}

	newMemoryBankWithStore = func(storepkg.VectorStore) *gomemory.MemoryBank {
		return gomemory.NewMemoryBankWithStore(fakeStore)
	}

	newSessionMemory = func(bank *gomemory.MemoryBank, size int) *gomemory.SessionMemory {
		return gomemory.NewSessionMemory(bank, size)
	}

	opts := gomemory.DefaultOptions()
	module := InPostgresMemory(context.Background(), 0, "", goembed.DummyEmbedder{}, &opts)
	bundle, err := module.provider(context.Background())
	if err != nil {
...
func TestInMongoMemoryCachesError(t *testing.T) {
	...
	var calls int32
	newMongoStore = func(context.Context, string, string, string) (*storepkg.MongoStore, error) {
		atomic.AddInt32(&calls, 1)
		return nil, errors.New("connect failed")
	}

	opts := gomemory.DefaultOptions()
	module := InMongoMemory(context.Background(), 4, "mongodb://invalid", "db", "collection", goembed.DummyEmbedder{}, &opts)
	provider := module.provider
...
func TestInMongoMemorySuccess(t *testing.T) {
	...
	opts := gomemory.DefaultOptions()
	module := InMongoMemory(context.Background(), 0, "", "", "", goembed.DummyEmbedder{}, &opts)
	bundle, err := module.provider(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestInNeo4jMemoryNilFactory(t *testing.T) {
	opts := gomemory.DefaultOptions()
	module := InNeo4jMemory(context.Background(), 4, nil, goembed.DummyEmbedder{}, &opts)
	provider := module.provider
...
func TestInNeo4jMemoryNilStore(t *testing.T) {
	opts := gomemory.DefaultOptions()
	module := InNeo4jMemory(context.Background(), 4, func(context.Context) (*storepkg.Neo4jStore, error) {
		return nil, nil
	}, goembed.DummyEmbedder{}, &opts)
	provider := module.provider
...
func TestInNeo4jMemorySuccess(t *testing.T) {
	opts := gomemory.DefaultOptions()
	module := InNeo4jMemory(context.Background(), 0, func(context.Context) (*storepkg.Neo4jStore, error) {
		return &storepkg.Neo4jStore{}, nil
	}, goembed.DummyEmbedder{}, &opts)
	bundle, err := module.provider(context.Background())
	if err != nil {
