...
type AgentDevelopmentKit struct {
	...
	memoryInstance   *gomemory.SessionMemory
...
func (k *AgentDevelopmentKit) NewSharedSession(ctx context.Context, local string, spaces ...string) (*gomemory.SharedSession, error) {
	if err := k.Bootstrap(ctx); err != nil {
		return nil, err
	}
	...
	if instance != nil {
		return gomemory.NewSharedSession(instance, local, spaces...), nil
	}
	...
	if factory == nil {
		factory = func(local string, spaces ...string) *gomemory.SharedSession {
			return gomemory.NewSharedSession(bundle.Session, local, spaces...)
		}
	}
