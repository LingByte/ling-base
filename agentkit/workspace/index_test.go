...
`), 0o644); err != nil {
	...
	if got := idx.Module(); got != "example.com/demo" {
	...
	if len(results) == 0 || results[0].Symbol.Name != "Authenticate" {
		t.Fatalf("unexpected search results: %#v", results)
	}
	if results[0].Symbol.Kind != SymbolMethod {
		t.Fatalf("kind = %q, want method", results[0].Symbol.Kind)
	}
...
func TestIndexIgnoresDirectoriesAndUnsupportedFiles(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "README.md"), []byte("main"), 0o644); err != nil {
		t.Fatal(err)
	}
