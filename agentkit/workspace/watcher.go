...
func (w *Watcher) Run(ctx context.Context) error {
	...
	for {
		...
		case <-ticker.C:
			...
			if len(changes) > 0 {
				if err := w.Index.Update(ctx, changes); err != nil {
					return err
				}
			}
			state = next
...
func (w *Watcher) snapshot() map[string]fileStamp {
	...
	_ = filepath.Walk(w.Index.root, func(path string, info fs.FileInfo, err error) error {
		...
		if info.IsDir() {
			if path != w.Index.root && w.Index.ignored(path) {
				return filepath.SkipDir
			}
			return nil
		}
		if !w.Index.supported(path) || info.Size() > w.Index.config.MaxFileSize {
			return nil
		}
		rel, err := filepath.Rel(w.Index.root, path)
		if err != nil {
			return nil
		}
