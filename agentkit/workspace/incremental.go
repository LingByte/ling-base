...
func (i *Index) Update(ctx context.Context, changes []FileChange) error {
	...
	for _, change := range changes {
		...
		if i.ignored(path) || !i.supported(path) {
			continue
		}
		st, err := os.Stat(path)
		...
		if st.IsDir() || st.Size() > i.config.MaxFileSize {
			i.removeFile(rel)
			continue
		}
		i.removeFile(rel)
		if err := i.indexFile(path, st.Size()); err != nil {
			return err
		}
	}
	...
	if i.embedder != nil {
		for _, change := range changes {
			...
			if filepath.IsAbs(change.Path) {
				if rel, err := filepath.Rel(i.root, change.Path); err == nil {
					path = filepath.ToSlash(rel)
				}
			}
			if change.Kind == ChangeDeleted {
				delete(i.vectors, path)
				continue
			}
			if f, ok := i.files[path]; ok {
				...
				if len(v) == 0 {
					...
					i.vectors[path] = append([]float32(nil), v...)
				}
			}
		}
	}
...
func (i *Index) removeFile(path string) {
	delete(i.files, filepath.ToSlash(path))
	delete(i.imports, filepath.ToSlash(path))
	delete(i.vectors, filepath.ToSlash(path))
}
func (i *Index) rebuildSymbolIndex() {
	...
	for _, s := range i.symbols {
		...
		i.byName[k] = append(i.byName[k], s)
	}
}
