...
func (p RateLimitPolicy) Wrap(next gomodel.Agent) (gomodel.Agent, error) {
	if next == nil {
...
func (a *rateLimitAgent) wrappedModel() gomodel.Agent { return a.next }
...
func (a *rateLimitAgent) GenerateWithFiles(ctx context.Context, prompt string, files []gomodel.File) (any, error) {
	if err := a.acquire(ctx); err != nil {
		return nil, err
	}
...
func (a *rateLimitAgent) GenerateStream(ctx context.Context, prompt string) (<-chan gomodel.StreamChunk, error) {
	if err := a.acquire(ctx); err != nil {
		return nil, err
	}
...
func (a *rateLimitAgent) GenerateWithTools(ctx context.Context, prompt string, tools []gomodel.ToolDefinition) (gomodel.ToolCallResponse, error) {
