...
func (p TimeoutPolicy) Wrap(next gomodel.Agent) (gomodel.Agent, error) {
	if next == nil {
...
func (a *timeoutAgent) wrappedModel() gomodel.Agent { return a.next }
...
func (a *timeoutAgent) GenerateWithFiles(ctx context.Context, prompt string, files []gomodel.File) (any, error) {
	return timeoutCall(ctx, a.duration, func(callCtx context.Context) (any, error) {
		return a.next.GenerateWithFiles(callCtx, prompt, files)
	})
}

func (a *timeoutAgent) GenerateWithTools(ctx context.Context, prompt string, tools []gomodel.ToolDefinition) (gomodel.ToolCallResponse, error) {
	native, err := nativeModel(a.next)
	if err != nil {
		return gomodel.ToolCallResponse{}, err
	}
	return timeoutCall(ctx, a.duration, func(callCtx context.Context) (gomodel.ToolCallResponse, error) {
		return native.GenerateWithTools(callCtx, prompt, tools)
	})
}

func (a *timeoutAgent) GenerateStream(ctx context.Context, prompt string) (<-chan gomodel.StreamChunk, error) {
	if ctx == nil {
	...
	type streamResult struct {
		stream <-chan gomodel.StreamChunk
		err    error
	}
	...
	var inner <-chan gomodel.StreamChunk
	select {
	case <-callCtx.Done():
		cancel()
	...
	out := make(chan gomodel.StreamChunk, 16)
	go func() {
		defer close(out)
		defer cancel()
...
func sendStreamChunk(ctx context.Context, out chan<- gomodel.StreamChunk, chunk gomodel.StreamChunk) bool {
	select {
	case out <- chunk:
		return true
	case <-ctx.Done():
