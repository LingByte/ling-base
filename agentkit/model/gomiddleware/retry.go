...
func (p RetryPolicy) Wrap(next gomodel.Agent) (gomodel.Agent, error) {
	if next == nil {
...
func (a *retryAgent) wrappedModel() gomodel.Agent { return a.next }
...
func (a *retryAgent) GenerateWithFiles(ctx context.Context, prompt string, files []gomodel.File) (any, error) {
	return retryCall(ctx, a.config, func(callCtx context.Context) (any, error) {
		return a.next.GenerateWithFiles(callCtx, prompt, files)
	})
}

func (a *retryAgent) GenerateStream(ctx context.Context, prompt string) (<-chan gomodel.StreamChunk, error) {
	return retryCall(ctx, a.config, func(callCtx context.Context) (<-chan gomodel.StreamChunk, error) {
		return a.next.GenerateStream(callCtx, prompt)
	})
}

func (a *retryAgent) GenerateWithTools(ctx context.Context, prompt string, tools []gomodel.ToolDefinition) (gomodel.ToolCallResponse, error) {
	native, err := nativeModel(a.next)
	if err != nil {
		return gomodel.ToolCallResponse{}, err
	}
	return retryCall(ctx, a.config, func(callCtx context.Context) (gomodel.ToolCallResponse, error) {
...
func defaultShouldRetry(ctx context.Context, err error) bool {
	...
	return !errors.Is(err, ErrRateLimitExceeded) &&
		...
		!errors.Is(err, gomodel.ErrToolCallingUnsupported)
}
