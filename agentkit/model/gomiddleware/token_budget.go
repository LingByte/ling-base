...
func (p TokenBudgetPolicy) Wrap(next gomodel.Agent) (gomodel.Agent, error) {
	if next == nil {
...
func (a *tokenBudgetAgent) wrappedModel() gomodel.Agent { return a.next }
...
func (a *tokenBudgetAgent) GenerateWithFiles(ctx context.Context, prompt string, files []gomodel.File) (any, error) {
	budget := a.budget(ctx)
	if budget == nil {
...
func (a *tokenBudgetAgent) GenerateWithTools(ctx context.Context, prompt string, tools []gomodel.ToolDefinition) (gomodel.ToolCallResponse, error) {
...
func (a *tokenBudgetAgent) GenerateStream(ctx context.Context, prompt string) (<-chan gomodel.StreamChunk, error) {
	budget := a.budget(ctx)
	if budget == nil {
	...
	go func() {
		...
		for {
			...
			case <-streamCtx.Done():
				if ctx.Err() != nil {
					out <- gomodel.StreamChunk{Done: true, FullText: forwarded.String(), Err: ctx.Err()}
				}
				return
			case chunk, ok := <-inner:
				if !ok {
				...
				if err := budget.chargeOutput(increment); err != nil {
					cancel()
					out <- gomodel.StreamChunk{Done: true, FullText: forwarded.String(), Err: err}
					return
				}
				charged += increment
...
func estimateFileTokens(budget *TokenBudget, files []gomodel.File) int64 {
	var total int64
...
func fileLooksTextual(file gomodel.File) bool {
