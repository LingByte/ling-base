...
type stubAgent struct {
	...
	generateWithFiles func(context.Context, string, []gomodel.File) (any, error)
	generateStream    func(context.Context, string) (<-chan gomodel.StreamChunk, error)
}
...
func (s *stubAgent) GenerateWithFiles(ctx context.Context, prompt string, files []gomodel.File) (any, error) {
...
func (s *stubAgent) GenerateStream(ctx context.Context, prompt string) (<-chan gomodel.StreamChunk, error) {
	if s.generateStream != nil {
		return s.generateStream(ctx, prompt)
	}
	out := make(chan gomodel.StreamChunk, 1)
	out <- gomodel.StreamChunk{Delta: "ok", FullText: "ok", Done: true}
	close(out)
	return out, nil
}
...
type nativeStubAgent struct {
	*stubAgent
	generateWithTools func(context.Context, string, []gomodel.ToolDefinition) (gomodel.ToolCallResponse, error)
}

func (s *nativeStubAgent) GenerateWithTools(ctx context.Context, prompt string, tools []gomodel.ToolDefinition) (gomodel.ToolCallResponse, error) {
	if s.generateWithTools != nil {
		return s.generateWithTools(ctx, prompt, tools)
	}
	return gomodel.ToolCallResponse{Content: "native"}, nil
}
...
func (p recordingPolicy) Wrap(next gomodel.Agent) (gomodel.Agent, error) {
...
func (a *recordingAgent) GenerateWithFiles(ctx context.Context, prompt string, files []gomodel.File) (any, error) {
	return a.next.GenerateWithFiles(ctx, prompt, files)
}

func (a *recordingAgent) GenerateStream(ctx context.Context, prompt string) (<-chan gomodel.StreamChunk, error) {
...
func TestRetryPolicyRetriesStreamSetupOnly(t *testing.T) {
	var calls atomic.Int32
	base := &stubAgent{generateStream: func(context.Context, string) (<-chan gomodel.StreamChunk, error) {
		if calls.Add(1) == 1 {
...
func TestTimeoutPolicyCoversStreamLifetime(t *testing.T) {
	base := &stubAgent{generateStream: func(ctx context.Context, _ string) (<-chan gomodel.StreamChunk, error) {
		out := make(chan gomodel.StreamChunk, 1)
		out <- gomodel.StreamChunk{Delta: "partial"}
		go func() {
			defer close(out)
			<-ctx.Done()
		}()
	...
	var chunks []gomodel.StreamChunk
...
func TestTokenBudgetAccountsInputOutputAndFiles(t *testing.T) {
	...
	base := &stubAgent{generateWithFiles: func(context.Context, string, []gomodel.File) (any, error) {
		return "z", nil
	}}
	wrapped, err := Wrap(base, TokenBudgetPolicy{Budget: budget})
	if err != nil {
...
func TestTokenBudgetStopsStreamBeforeExceededChunk(t *testing.T) {
	...
	base := &stubAgent{generateStream: func(context.Context, string) (<-chan gomodel.StreamChunk, error) {
		out := make(chan gomodel.StreamChunk, 3)
		out <- gomodel.StreamChunk{Delta: "ab"}
		out <- gomodel.StreamChunk{Delta: "cd"}
		out <- gomodel.StreamChunk{Done: true, FullText: "abcd"}
		close(out)
		return out, nil
	}}
	wrapped, err := Wrap(base, TokenBudgetPolicy{Budget: budget})
...
func TestPoliciesPreserveNativeToolCalling(t *testing.T) {
	...
	native, ok := wrapped.(gomodel.ToolCallingAgent)
	if !ok {
		t.Fatal("wrapped model lost ToolCallingAgent")
	}
	result, err := native.GenerateWithTools(context.Background(), "call", []gomodel.ToolDefinition{{Name: "echo"}})
	if err != nil {
...
func TestPoliciesReportUnsupportedNativeToolCallingWithoutCharging(t *testing.T) {
	...
	native := wrapped.(gomodel.ToolCallingAgent)
