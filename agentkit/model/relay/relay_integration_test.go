...
func TestRelayModelLive(t *testing.T) {
	...
	req := &model.Request{
		Messages: []model.Message{
			model.NewSystemMessage("You are a helpful assistant. Answer concisely."),
			model.NewUserMessage("What is 2+2? Answer in one word."),
		},
...
func TestRelayModelProviderLive(t *testing.T) {
	...
	req := &model.Request{
		Messages: []model.Message{
			model.NewUserMessage("Say hello in one word."),
		},
		GenerationConfig: model.GenerationConfig{
			Stream:    true,
			MaxTokens: ptrInt(50),
