...
type Summarizer interface {
	Summarize(ctx context.Context, cluster []gomodel.MemoryRecord) (string, error)
}

// HeuristicSummarizer produces deterministic summaries suitable for tests.
type HeuristicSummarizer struct{}

func (HeuristicSummarizer) Summarize(_ context.Context, cluster []gomodel.MemoryRecord) (string, error) {
	if len(cluster) == 0 {
		return "", nil
	}
