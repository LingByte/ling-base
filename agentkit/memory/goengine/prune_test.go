...
func (s *recordingDeleteStore) Iterate(ctx context.Context, fn func(gomodel.MemoryRecord) bool) error {
	s.iterating = true
	defer func() { s.iterating = false }()
	return s.InMemoryStore.Iterate(ctx, func(rec gomodel.MemoryRecord) bool {
		keepGoing := fn(rec)
		s.observeSpool()
		return keepGoing
	})
...
func (s *iterateErrorStore) Iterate(ctx context.Context, fn func(gomodel.MemoryRecord) bool) error {
	called := false
	if err := s.InMemoryStore.Iterate(ctx, func(rec gomodel.MemoryRecord) bool {
		called = true
