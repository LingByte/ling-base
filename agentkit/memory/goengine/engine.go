...
import (
	...
	"github.com/LingByte/ling-base/agentkit/memory/goembed"
...
type Engine struct {
	store      storepkg.VectorStore
	opts       Options
	embedder   goembed.Embedder
...
func NewEngine(store storepkg.VectorStore, opts Options) *Engine {
	opts = opts.withDefaults()
	engine := &Engine{
		store:      store,
		opts:       opts,
		embedder:   goembed.AutoEmbedder(),
...
func (e *Engine) WithEmbedder(embedder goembed.Embedder) *Engine {
	if embedder != nil {
		e.embedder = embedder
	}
	return e
...
func (e *Engine) Store(ctx context.Context, sessionID, content string, metadata map[string]any) (gomodel.MemoryRecord, error) {
	if e.store == nil {
		return gomodel.MemoryRecord{}, errors.New("memory engine has no store")
	}
	embedding, err := e.embed(ctx, content)
	if err != nil {
		return gomodel.MemoryRecord{}, fmt.Errorf("embed content: %w", err)
	}
	now := e.clock().UTC()
	if metadata == nil {
	...
	edges := gomodel.SanitizeGraphEdges(metadata)
	...
	if err != nil {
		return gomodel.MemoryRecord{}, err
	}
	for _, cand := range candidates {
		sim := gomodel.MaxCosineSimilarity(embedding, cand)
	...
	newRecord := gomodel.MemoryRecord{
		SessionID:    sessionID,
		Content:      content,
		Metadata:     gomodel.StringFromAny(metadata),
		Embedding:    embedding,
		Importance:   importance,
		Source:       gomodel.StringFromAny(metadata["source"]),
		CreatedAt:    now,
		LastEmbedded: now,
	}
	...
	stored.Metadata = gomodel.StringFromAny(metadata)
	stored.Summary = gomodel.StringFromAny(metadata["summary"])
	stored.Importance = importance
	if graphStore, ok := e.store.(storepkg.GraphStore); ok {
...
func (e *Engine) Retrieve(ctx context.Context, sessionID, query string, limit int) ([]gomodel.MemoryRecord, error) {
	if e.store == nil {
	...
	similarityQuery := gomodel.NewCosineQuery(embedding)
	if graphStore, ok := e.store.(storepkg.GraphStore); ok && e.opts.GraphNeighborhoodLimit > 0 {
...
func (e *Engine) embed(ctx context.Context, text string) ([]float32, error) {
	if e.embedder == nil {
		e.embedder = goembed.AutoEmbedder()
	}
	vec, err := e.embedder.Embed(ctx, text)
	if err != nil || len(vec) == 0 {
		return goembed.DummyEmbedding(text), nil
	}
	return vec, nil
}

func (e *Engine) reembedOnDrift(ctx context.Context, records []gomodel.MemoryRecord) error {
	for _, rec := range records {
		...
		sim := gomodel.MaxCosineSimilarity(vec, rec)
...
func (e *Engine) populateSummaries(ctx context.Context, records []gomodel.MemoryRecord) error {
...
func (e *Engine) clusterSummary(ctx context.Context, records []gomodel.MemoryRecord, target gomodel.MemoryRecord) (string, error) {
	clusters := clusterRecords(records, e.opts.ClusterSimilarity)
	for _, cluster := range clusters {
...
func importanceScore(content string, metadata map[string]any) float64 {
	if val := gomodel.FloatFromAny(metadata["importance"]); val > 0 {
		return clamp(val, 0, 1)
	}
...
func flattenMetadata(meta map[string]any) []string {
	...
	visit = func(v any) {
		...
		default:
			if s := strings.TrimSpace(gomodel.StringFromAny(val)); s != "" && s != "[]" && s != "{}" && s != "map[]" {
...
func mmrSelect(records []gomodel.MemoryRecord, query []float32, limit int, lambda float64) []gomodel.MemoryRecord {
	if limit >= len(records) {
		out := make([]gomodel.MemoryRecord, len(records))
		copy(out, records)
		return out
	}
	if lambda < 0 {
	...
	remaining := make([]gomodel.MemoryRecord, len(records))
	copy(remaining, records)
	selected := make([]gomodel.MemoryRecord, 0, limit)
	for len(selected) < limit && len(remaining) > 0 {
		...
		for i, cand := range remaining {
			...
			if relevance == 0 {
				relevance = gomodel.MaxCosineSimilarity(query, cand)
			}
			var maxSim float64
			for _, sel := range selected {
				if sim := gomodel.RecordSimilarity(cand, sel); sim > maxSim {
					maxSim = sim
				}
			}
...
func clusterRecords(records []gomodel.MemoryRecord, threshold float64) [][]gomodel.MemoryRecord {
	if threshold <= 0 {
		threshold = 0.8
	}
	type cluster struct {
		centroid []float64
		records  []gomodel.MemoryRecord
	}
	var clusters []cluster
	for _, rec := range records {
		vec := gomodel.RepresentativeEmbedding(rec)
		if len(vec) == 0 {
			continue
		}
		placed := false
		for i := range clusters {
			sim := gomodel.CosineSimilarity(vec, float32Slice(clusters[i].centroid))
			if sim >= threshold {
		...
		if !placed {
			clusters = append(clusters, cluster{
				...
				records:  []gomodel.MemoryRecord{rec},
			})
		}
	}
	result := make([][]gomodel.MemoryRecord, len(clusters))
