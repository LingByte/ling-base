...
func (s *Store) SearchMemory(ctx context.Context, sessionID string, queryEmbedding []float32, limit int) ([]gomodel.MemoryRecord, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	...
	result := make([]gomodel.MemoryRecord, len(scored))
...
func (s *Store) Iterate(ctx context.Context, fn func(gomodel.MemoryRecord) bool) error {
	if err := ctx.Err(); err != nil {
		return err
	}
