...
func (s *InMemoryStore) SearchMemory(_ context.Context, sessionID string, queryEmbedding []float32, limit int) ([]gomodel.MemoryRecord, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if limit <= 0 {
		return nil, nil
	...
	result := make([]gomodel.MemoryRecord, len(scoredRecords))
...
func calculateRecordMagnitudes(rec gomodel.MemoryRecord) recordVectorMagnitudes {
	magnitudes := recordVectorMagnitudes{
		embedding: gomodel.VectorMagnitude(rec.Embedding),
	}
	if len(rec.EmbeddingMatrix) > 0 {
		magnitudes.matrix = make([]float64, len(rec.EmbeddingMatrix))
		for i, vector := range rec.EmbeddingMatrix {
			magnitudes.matrix[i] = gomodel.VectorMagnitude(vector)
		}
	}
	return magnitudes
}
...
func (s *InMemoryStore) Iterate(_ context.Context, fn func(gomodel.MemoryRecord) bool) error {
	s.mu.RLock()
	defer s.mu.RUnlock()
