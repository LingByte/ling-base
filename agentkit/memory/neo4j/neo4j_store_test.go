...
func TestNeo4jStoreUpsertGraph(t *testing.T) {
	...
	edges := []gomodel.GraphEdge{
		{Target: 7, Type: gomodel.EdgeFollows},
		{Target: 8, Type: gomodel.EdgeExplains},
		{Target: 0, Type: gomodel.EdgeExplains}, // invalid and skipped
	}
	...
	if got := batch[0]["edge_type"].(string); got != string(gomodel.EdgeFollows) {
...
func TestNeo4jUnavailable(t *testing.T) {
	...
	if err := store.UpsertGraph(context.Background(), gomodel.MemoryRecord{ID: 1}, nil); !errors.Is(err, ErrNeo4jUnavailable) {
