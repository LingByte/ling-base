...
func (qs *QdrantStore) StoreMemory(ctx context.Context, sessionID, content string, metadata map[string]any, embedding []float32) error {
	...
	graphEdges := gomodel.SanitizeGraphEdges(metadata)
	...
	payload := map[string]any{
		...
		"content":       content,
		"metadata":      gomodel.DecodeMetadata(record.Metadata),
	...
	if len(record.EmbeddingMatrix) > 0 {
		payload[gomodel.EmbeddingMatrixKey] = record.EmbeddingMatrix
	}
...
func (qs *QdrantStore) SearchMemory(ctx context.Context, sessionID string, queryEmbedding []float32, limit int) ([]gomodel.MemoryRecord, error) {
	if qs == nil {
	...
	results := make([]gomodel.MemoryRecord, 0, len(resp.Result))
	for _, point := range resp.Result {
		id, _ := parseQdrantID(point.ID)
		meta := mapFromPayload(point.Payload)
		metaMap, _ := meta["metadata"].(map[string]any)
		if metaMap == nil {
			metaMap = gomodel.DecodeMetadata(encodeMetadata(meta["metadata"]))
		}
		record := gomodel.MemoryRecord{
			ID:           id,
			SessionID:    gomodel.StringFromAny(meta["session_id"]),
			Content:      gomodel.StringFromAny(meta["content"]),
			Metadata:     encodeMetadata(meta["metadata"]),
			Embedding:    point.Vector,
			Importance:   gomodel.FloatFromAny(meta["importance"]),
			Source:       gomodel.StringFromAny(meta["source"]),
			Summary:      gomodel.StringFromAny(meta["summary"]),
			CreatedAt:    gomodel.TimeFromAny(meta["created_at"]),
			LastEmbedded: gomodel.TimeFromAny(meta["last_embedded"]),
		}
		gomodel.HydrateRecordFromMetadata(&record, metaMap)
		if len(record.EmbeddingMatrix) == 0 {
			if matrix := gomodel.DecodeEmbeddingMatrix(meta[gomodel.EmbeddingMatrixKey]); len(matrix) > 0 {
				record.EmbeddingMatrix = matrix
			} else if matrix := gomodel.DecodeEmbeddingMatrix(point.Payload[gomodel.EmbeddingMatrixKey]); len(matrix) > 0 {
				record.EmbeddingMatrix = matrix
			}
		}
		if record.Space == "" {
			record.Space = gomodel.StringFromAny(meta["space"])
			if record.Space == "" {
				record.Space = record.SessionID
			}
		}
		if len(record.GraphEdges) == 0 {
			record.GraphEdges = gomodel.ValidGraphEdges(metaMap)
		}
...
func (qs *QdrantStore) Iterate(ctx context.Context, fn func(gomodel.MemoryRecord) bool) error {
	if qs == nil {
		return nil
	}
	...
	for page := 0; page < maxPages; page++ {
		...
		for _, point := range resp.Result.Points {
			...
			metaMap := gomodel.DecodeMetadata(rec.Metadata)
			gomodel.HydrateRecordFromMetadata(&rec, metaMap)
			if len(rec.EmbeddingMatrix) == 0 {
				if matrix := gomodel.DecodeEmbeddingMatrix(metaMap[gomodel.EmbeddingMatrixKey]); len(matrix) > 0 {
...
func (qs *QdrantStore) UpsertGraph(ctx context.Context, record gomodel.MemoryRecord, edges []gomodel.GraphEdge) error {
...
func (qs *QdrantStore) Neighborhood(ctx context.Context, seedIDs []int64, hops, limit int) ([]gomodel.MemoryRecord, error) {
	...
	results := make([]gomodel.MemoryRecord, 0, len(points))
...
func mapFromPayload(payload map[string]any) map[string]any {
	...
	if md, ok := payload["metadata"]; ok {
		switch t := md.(type) {
		case string:
			payload["metadata"] = gomodel.DecodeMetadata(t)
		case json.RawMessage:
			payload["metadata"] = gomodel.DecodeMetadata(string(t))
		case map[string]any:
			// already a map
		default:
...
func extractEdgesFromPayload(payload map[string]any) []gomodel.GraphEdge {
	if payload == nil {
		return nil
	}
	if raw, ok := payload["graph_edges"]; ok {
		return gomodel.ValidGraphEdges(map[string]any{"graph_edges": raw})
	}
	if meta, ok := payload["metadata"].(map[string]any); ok {
		return gomodel.ValidGraphEdges(meta)
	}
	return nil
}
