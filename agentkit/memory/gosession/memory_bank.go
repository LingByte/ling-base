...
type SessionMemory struct {
	Bank          *MemoryBank
	shortTerm     map[string][]gomodel.MemoryRecord
	mu            sync.RWMutex
	shortTermSize int
...
func NewMemoryBank(ctx context.Context, connStr string) (*MemoryBank, error) {
	s, err := storepkg.NewPostgresStore(ctx, connStr)
	if err != nil {
		return nil, err
	}
...
func NewMemoryBankWithStore(s storepkg.VectorStore) *MemoryBank {
...
func NewSessionMemory(bank *MemoryBank, shortTermSize int) *SessionMemory {
	return &SessionMemory{
		Bank:          bank,
		shortTerm:     make(map[string][]gomodel.MemoryRecord),
...
func (sm *SessionMemory) AddShortTerm(sessionID, content, metadata string, embedding []float32) {
	...
	record := gomodel.MemoryRecord{SessionID: sessionID, Space: sessionID, Content: content, Metadata: metadata, Embedding: embedding}
...
// FlushToLongTerm writes the short-term cache to the configured vector storepkg.
func (sm *SessionMemory) FlushToLongTerm(ctx context.Context, sessionID string) error {
	...
	for _, r := range records {
		if sm.Engine != nil {
			meta := gomodel.DecodeMetadata(r.Metadata)
...
func (sm *SessionMemory) Embed(ctx context.Context, text string) ([]float32, error) {
	if sm.Embedder == nil {
		sm.Embedder = goembed.AutoEmbedder()
	}
...
func (sm *SessionMemory) RetrieveContext(ctx context.Context, sessionID, query string, limit int) ([]gomodel.MemoryRecord, error) {
	var longTerm []gomodel.MemoryRecord
	if sm.Engine != nil {
...
func (sm *SessionMemory) WithEmbedder(e goembed.Embedder) *SessionMemory {
	if e != nil {
		sm.Embedder = e
	}
	return sm
...
func (mb *MemoryBank) SearchMemory(ctx context.Context, sessionID string, queryEmbedding []float32, limit int) ([]gomodel.MemoryRecord, error) {
	if mb == nil || mb.Store == nil {
		return nil, nil
	}
...
func (mb *MemoryBank) CreateSchema(ctx context.Context, schemaPath string) error {
	...
	initializer, ok := mb.Store.(storepkg.SchemaInitializer)
	if !ok {
		return nil
	}
...
func (sm *SessionMemory) ExportShortTerm() map[string][]gomodel.MemoryRecord {
	sm.mu.RLock()
	defer sm.mu.RUnlock()

	copyMap := make(map[string][]gomodel.MemoryRecord)
	for k, v := range sm.shortTerm {
		records := make([]gomodel.MemoryRecord, len(v))
		copy(records, v)
		copyMap[k] = records
	}
	return copyMap
...
func (sm *SessionMemory) ImportShortTerm(data map[string][]gomodel.MemoryRecord) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	sm.shortTerm = data
}
