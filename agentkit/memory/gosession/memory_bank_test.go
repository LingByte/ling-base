...
type stubVectorStore struct {
	mu           sync.Mutex
	stored       []storedMemory
	searchResp   []gomodel.MemoryRecord
	storeErr     error
	searchErr    error
	createSchema []string
	closed       bool
...
func (s *stubVectorStore) SearchMemory(_ context.Context, _ string, _ []float32, _ int) ([]gomodel.MemoryRecord, error) {
	if s.searchErr != nil {
...
func (s *stubVectorStore) Iterate(context.Context, func(gomodel.MemoryRecord) bool) error { return nil }
...
var _ memorystore.VectorStore = (*stubVectorStore)(nil)
var _ memorystore.SchemaInitializer = (*stubVectorStore)(nil)
...
func TestSessionMemoryEmbedFallback(t *testing.T) {
	...
	dummy := goembed.DummyEmbedding("text")
...
func TestSessionMemoryRetrieveContextCombinesSources(t *testing.T) {
	svs := &stubVectorStore{searchResp: []gomodel.MemoryRecord{{Content: "long"}}}
	bank := NewMemoryBankWithStore(svs)
	sm := NewSessionMemory(bank, 2)
	sm.WithEmbedder(stubEmbedder{vec: []float32{1}})
