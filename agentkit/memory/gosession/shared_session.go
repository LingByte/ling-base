...
func (ss *SharedSession) StoreLongTo(ctx context.Context, sessionID, content string, metadata map[string]any) (gomodel.MemoryRecord, error) {
	if ss == nil || ss.base == nil {
		return gomodel.MemoryRecord{}, errors.New("nil shared session")
	}
	if strings.TrimSpace(content) == "" {
		return gomodel.MemoryRecord{}, errors.New("content is empty")
	}
	if sessionID != ss.local && !ss.canWrite(sessionID) {
		return gomodel.MemoryRecord{}, ErrSpaceForbidden
	}
	if ss.base.Engine != nil {
		return ss.base.Engine.Store(ctx, sessionID, content, metadata)
	}
	// Bank-only path: compute embedding and storepkg.
	...
	return gomodel.MemoryRecord{SessionID: sessionID, Content: content, Metadata: string(metaBytes), Embedding: emb}, nil
}

// BroadcastLong writes a long-term memory to the local session and all spaces.
func (ss *SharedSession) BroadcastLong(ctx context.Context, content string, metadata map[string]any) ([]gomodel.MemoryRecord, error) {
	if ss == nil || ss.base == nil {
		return nil, errors.New("nil shared session")
	}
	targets := ss.allowedWriteSessions()
	out := make([]gomodel.MemoryRecord, 0, len(targets))
...
func (ss *SharedSession) Retrieve(ctx context.Context, query string, limit int) ([]gomodel.MemoryRecord, error) {
...
func (ss *SharedSession) RetrieveShared(ctx context.Context, query string, limit int) ([]gomodel.MemoryRecord, error) {
	return ss.retrieve(ctx, query, limit, false)
}

func (ss *SharedSession) retrieve(ctx context.Context, query string, limit int, includeLocal bool) ([]gomodel.MemoryRecord, error) {
	...
	ss.base.mu.RLock()
	var short []gomodel.MemoryRecord
	...
	push := func(dst *[]gomodel.MemoryRecord, rec gomodel.MemoryRecord) {
		if rec.ID != 0 {
	...
	var merged []gomodel.MemoryRecord
	for _, r := range short {
		push(&merged, r)
	}
	for _, r := range long {
