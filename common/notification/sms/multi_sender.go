...
// MultiSender sends SMS through a set of channels with round-robin
// selection and automatic failover to the next enabled channel.
...
func (m *MultiSender) Send(ctx context.Context, req SendRequest) (*SendResult, error) {
	...
	if len(enabled) == 0 {
		return nil, fmt.Errorf("sms: no enabled channels")
	}
	...
	if lastErr == nil {
		lastErr = fmt.Errorf("sms: all channels failed")
	}
	if lastResult == nil {
		lastResult = &SendResult{Status: "failed", Error: lastErr.Error()}
	}
	return lastResult, fmt.Errorf("sms: all channels failed: %w", lastErr)
}
