...
func splitByHeuristicsImpl(text string, cfg SplitterConfig, _ *DocProfile) []splitChunk {
	if text == "" {
		return nil
	}
	runes := []rune(text)
...
// appendChunk slices runes[start:end] into a splitChunk and appends it to out.
...
// splitChunk.EmbeddingContent.
func appendChunk(out []splitChunk, runes []rune, start, end int, seq *int) []splitChunk {
	if end <= start {
		return out
	}
	...
	c := splitChunk{Content: raw, Seq: *seq, Start: start, End: end}
	*seq++
	return append(out, c)
}
...
func appendOversizeBlock(out []splitChunk, runes []rune, start, end int, cfg SplitterConfig, seq *int) []splitChunk {
	if end <= start {
		return out
	}
