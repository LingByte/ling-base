// strategy.go is the public entry point for tiered text splitting.
// Callers invoke Split / SplitParentChild instead of the legacy
...
func Split(text string, cfg SplitterConfig) []splitChunk {
	if text == "" {
		return nil
	}
...
func SplitWithDiagnostics(text string, cfg SplitterConfig) ([]splitChunk, *Diagnostics) {
...
func SplitParentChild(text string, parentCfg, childCfg SplitterConfig) ParentChildResult {
	...
	for _, parent := range parents {
		...
		for _, sub := range subs {
			...
			children = append(children, ChildChunk{splitChunk: sub, ParentIndex: parentIndex})
			childSeq++
		}
	}
...
func runTier(tier StrategyTier, text string, cfg SplitterConfig, profile *DocProfile) []splitChunk {
	switch tier {
	case TierHeading:
...
var splitByHeadings = func(text string, cfg SplitterConfig, _ *DocProfile) []splitChunk {
	return SplitText(text, cfg)
}

// splitByHeuristics is overridden by heuristic_splitter.go. profile may be nil.
var splitByHeuristics = func(text string, cfg SplitterConfig, _ *DocProfile) []splitChunk {
	return SplitText(text, cfg)
}
