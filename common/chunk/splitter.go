...
// splitChunk represents a piece of split text with position tracking.
//
...
// preserved.
func (c splitChunk) EmbeddingContent() string {
...
func SplitText(text string, cfg SplitterConfig) []splitChunk {
	if text == "" {
		return nil
	}
...
func mergeUnits(units []splitUnit, chunkSize, chunkOverlap int) []splitChunk {
	if len(units) == 0 {
		return nil
	}
...
func buildChunk(units []splitUnit, seq int) splitChunk {
	var sb strings.Builder
	for _, u := range units {
		sb.WriteString(u.text)
	}
...
// ChildChunk extends splitChunk with a reference to its parent.
