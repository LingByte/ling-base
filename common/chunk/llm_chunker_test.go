...
// stubChatModel implements ChatModel for testing.
type stubChatModel struct {
	lastReq *relay.ChatRequest
	resp    string
}

func (m *stubChatModel) Name() string { return "stub" }

func (m *stubChatModel) Chat(ctx context.Context, req *relay.ChatRequest) (*relay.ChatResponse, error) {
	m.lastReq = req
	return &relay.ChatResponse{
		Choices: []relay.ChatChoice{{
			Message: relay.Message{Role: "assistant", Content: m.resp},
		}},
	}, nil
}
...
func TestRelayChatModel_NilClient(t *testing.T) {
	m := &RelayChatModel{}
	_, err := m.Chat(context.Background(), &relay.ChatRequest{})
	if err == nil {
		t.Error("expected error for nil client")
	}
}

func TestRelayChatModel_Name(t *testing.T) {
	m := NewRelayChatModel(nil, "test-model")
	if m.Name() != "test-model" {
		t.Errorf("expected 'test-model', got '%s'", m.Name())
	}
}
