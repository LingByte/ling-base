...
func splitByHeadingsImpl(text string, cfg SplitterConfig, profile *DocProfile) []splitChunk {
	if text == "" {
		return nil
	}
	if profile == nil {
	...
	for i, b := range bounds {
		...
		// The breadcrumb is delivered via splitChunk.ContextHeader (not Content)
...
func coalesceTinyChunks(in []splitChunk, chunkSize int) []splitChunk {
