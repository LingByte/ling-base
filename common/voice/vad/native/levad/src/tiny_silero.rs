...
impl TinySilero {
    ...
    pub fn load_weights(&mut self) -> Result<()> {
        const WEIGHTS: &[u8] = include_bytes!("../weights/silero_weights.bin");
    ...
    pub fn predict(&mut self, audio: &[f32]) -> f32 {
        ...
        for j in 0..HIDDEN_SIZE {
            // IFCO order
            let i_gate = super::utils::sigmoid(self.buf_gates[0 * chunk + j]);
            let f_gate = super::utils::sigmoid(self.buf_gates[1 * chunk + j]);
            let g_gate = super::utils::tanh(self.buf_gates[2 * chunk + j]); // Cell
            let o_gate = super::utils::sigmoid(self.buf_gates[3 * chunk + j]);

            let c_new = f_gate * self.c[0][j] + i_gate * g_gate;
            let h_val = o_gate * super::utils::tanh(c_new);
        ...
        let out = super::utils::sigmoid(sum);
