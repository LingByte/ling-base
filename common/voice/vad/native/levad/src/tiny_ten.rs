...
use super::{AudioFrame, PcmBuf, Samples};
use anyhow::Result;
...
impl LstmLayer {
    ...
    fn forward_optimized(&mut self, input: &[f32], hidden: &mut [f32], cell: &mut [f32]) {
        ...
        for i in 0..h_size {
            let i_gate = super::utils::sigmoid(self.gates_buffer[i]);
            let o_gate = super::utils::sigmoid(self.gates_buffer[i + h_size]);
            let f_gate = super::utils::sigmoid(self.gates_buffer[i + 2 * h_size]);
            let g_gate = super::utils::tanh(self.gates_buffer[i + 3 * h_size]);

            // c_t = f_t * c_{t-1} + i_t * g_t
            cell[i] = f_gate * cell[i] + i_gate * g_gate;

            // h_t = o_t * tanh(c_t)
            hidden[i] = o_gate * super::utils::tanh(cell[i]);
        }
    }
}
...
const WEIGHTS_BYTES: &[u8] = include_bytes!("../weights/tiny_tenvad.bin");
